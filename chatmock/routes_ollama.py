...
def _build_model_entry(model_id: str) -> Dict[str, Any]:
    metadata = deepcopy(OLLAMA_MODEL_METADATA)
    entry = {"name": model_id, "model": model_id}
...
def ollama_version() -> Response:
    ...
    version = current_app.config.get("OLLAMA_VERSION", DEFAULT_OLLAMA_VERSION)
...
def ollama_tags() -> Response:
    ...
    model_ids = list(OLLAMA_BASE_MODEL_IDS)
    if expose_variants:
        model_ids.extend(OLLAMA_VARIANT_MODEL_IDS)
    models = [_build_model_entry(model_id) for model_id in model_ids]
...
def ollama_show() -> Response:
    ...
    response_payload = deepcopy(OLLAMA_SHOW_RESPONSE)
    if verbose:
        _log_json("OUT POST /api/show", response_payload)
